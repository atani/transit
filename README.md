...
駅名は複数の候補に一致します。1 つの駅名が事業者ごとのフィードに分かれて存在し、さらに同名のバス停も一致するためです。`plan` は次の優先順で出発・到着の組を選びます。

1. 同じフィードの鉄道駅どうし（乗換なしで到達できる可能性が高い）
2. 別フィードの鉄道駅どうし（API が事業者をまたぐ乗換を計算）
3. 上記が無い場合は同じフィードの停留所、それも無ければ各先頭候補

鉄道駅をバス停より優先するのは、`押上` のような駅名がバス停にも一致し、そちらに解決すると終日かかるバス経路になることがあるためです。公共交通機関の経路が見つからず徒歩のみになった場合は、CLI が注記を表示します。
